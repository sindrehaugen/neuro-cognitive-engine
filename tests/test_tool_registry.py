...
_EXPECTED_STATIC_TOTAL = 307  # 292 hand-written tools + 8 FUNCTIONAL_LOCATION tree tools (Lane C Wave C-1) + 1 C17 site master data address-registry feed (Lane F Wave F-9) + 2 support action/timeline tools (Lane D Wave D-5) + 1 support on-call rota (Lane D Wave D-7) + 3 support summary/links tools (Lane D Wave D-6); see _C12_TOOL_NAMES above
...
def test_mutation_tools_count():
    ...
    +1 Lane D Wave D-6 support_link_ticket -> 133."""
...
def test_cacheable_tools_count():
    ...
    baseline + 5 Lane F Wave F-11..F-15 tools + 5 Wave C-1 FL tree reads + 1 Wave D-5 support timeline + 1 Wave D-7 on-call + 2 Wave D-6 support summary & links = 129."""
    ...
    assert len(CACHEABLE_TOOLS) >= 129, (
        f"Sanity floor: expected at least 129 cacheable tools, got {len(CACHEABLE_TOOLS)}."
    )
    assert len(hand_written_cacheable_tools) == 129, (
        "Hand-written (non-C12) cacheable tool count changed: expected 129, "
