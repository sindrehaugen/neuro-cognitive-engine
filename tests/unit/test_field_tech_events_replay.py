"""Unit tests for Field Tech Engine events, replay handlers, and Contract-A node ownership.

ML12-B7: Phase 5 - Events, Replay & Graph Ownership.
Verifies:
  1. EventType and VALID_EVENT_TYPES include all 6 field_tech event types:
  ...
  2. ForkedReplay handler registry coverage (_validate_handler_coverage) includes all field_tech events.
  3. Replay handler for field_tech events returns expected provenance replay dict.
  4. Contract-A node ownership in nce/config_data/node-ownership.json:
     - WORK_ORDER, FIELD_TECH_CHECKLIST, FIELD_TECH_TIME_ENTRY, FIELD_TECH_SCAN, FIELD_TECH_PHOTO
       are registered with owner_engine="field_tech" and transition=null.
     - Single-transition ratchet: none of these 5 node types have non-null transition rows.
  5. assert_owner / OwnershipError semantics against registered field_tech ownership rules:
     - 'field_tech' engine passes.
     - other engines ('support', 'sales', 'assets', 'system_design') are refused with OwnershipError.
...
_FIELD_TECH_EVENT_TYPES = frozenset(
    {
        "field_tech_work_order_created",
        "field_tech_work_order_assigned",
        "field_tech_checklist_completed",
...
_FIELD_TECH_NODE_TYPES = frozenset(
    {
        "WORK_ORDER",
...
def test_field_tech_event_types_defined_in_event_type_union() -> None:
    """All 6 field tech event types must be members of EventType and VALID_EVENT_TYPES."""
    all_types = frozenset(get_args(EventType))
    missing = _FIELD_TECH_EVENT_TYPES - all_types
    assert not missing, f"EventType missing field tech event types: {sorted(missing)}"

    missing_valid = _FIELD_TECH_EVENT_TYPES - VALID_EVENT_TYPES
    assert not missing_valid, f"VALID_EVENT_TYPES missing: {sorted(missing_valid)}"


def test_field_tech_replay_handler_coverage() -> None:
    """_validate_handler_coverage must pass and all field tech event types must have handlers in _HANDLER_REGISTRY."""
    # Must not raise ReplayHandlerMissingError
    _validate_handler_coverage()

    for et in _FIELD_TECH_EVENT_TYPES:
        assert et in _HANDLER_REGISTRY, f"No replay handler registered for {et!r}"


@pytest.mark.asyncio
async def test_field_tech_replay_handler_execution() -> None:
    """Invoking the registered replay handler for field tech events returns provenance result."""
    ...
    for et in _FIELD_TECH_EVENT_TYPES:
        ...
        row = _EventRow(
            ...
            agent_id="field-tech-agent",
            params={"work_order_id": str(uuid4()), "detail": "test"},
            result_summary=None,
...
def test_node_ownership_json_contains_field_tech_spine_nodes() -> None:
    """nce/config_data/node-ownership.json must contain whole-node ownership for field tech spine nodes."""
    ...
    for node_type in _FIELD_TECH_NODE_TYPES:
        ...
        row = rows[0]
        assert row["owner_engine"] == "field_tech", (
            f"expected owner_engine='field_tech' for {node_type}, got {row.get('owner_engine')!r}"
        )
...
def test_no_field_tech_node_type_has_conflicting_transition() -> None:
    """Verify that none of the field tech node types violate the null-vs-non-null transition ratchet."""
    raw = _OWNERSHIP_MAP_PATH.read_text(encoding="utf-8")
    data: dict[str, Any] = json.loads(raw)
    entries: list[dict[str, Any]] = data.get("ownership", [])
...
@pytest.mark.asyncio
async def test_assert_owner_field_tech_engine_contract() -> None:
    """Verify assert_owner permissions and refusals for Field Tech nodes."""
    ...
    try:
        # 1. 'field_tech' engine should pass for all field tech node types
        for node_type in _FIELD_TECH_NODE_TYPES:
            await assert_owner(mock_conn, ns_id, node_type, "field_tech")

        # 2. Non-field_tech engines ('support', 'sales', 'assets', 'system_design') should be refused with OwnershipError
        for node_type in _FIELD_TECH_NODE_TYPES:
        ...
        with pytest.raises(OwnershipError) as exc_info:
            await assert_owner(mock_conn, ns_id, "UNREGISTERED_FIELD_TECH_NODE_XYZ", "field_tech")
        err = exc_info.value
        assert err.node_type == "UNREGISTERED_FIELD_TECH_NODE_XYZ"
