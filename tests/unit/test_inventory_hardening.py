...
_TOTAL_TOOLS = 220  # 206 baseline + 6 economy (Wave E-1) + 1 sales signing request (Wave S-2a) + 1 decision feedback (Wave C10) + 2 product tools (Waves P-2 / P-3) + 1 project outcome (Wave PJ-1) + 1 resources allocation outcome (Wave RS-3) + 2 trust dial (Wave T-1)
_MUTATION_TOOLS = 93  # 87 baseline + 1 sales signing request + 1 decision feedback + 1 product spec ingest + 1 project record outcome + 1 resources allocation outcome + 1 trust dial set tier
_CACHEABLE_TOOLS = 84  # 82 baseline + 1 product golden record + 1 trust dial status
_ADMIN_ONLY_TOOLS = 54  # 49 baseline + 1 sales signing request + 1 decision feedback + 1 project record outcome + 1 resources allocation outcome + 1 trust dial set tier
_MIGRATION_TOOLS = 5
