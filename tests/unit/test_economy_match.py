...
Split per round-2 rule #3, mirroring ``test_procurement_tco.py``:
  (a) ALGORITHM tests — ported from Andreas's ``matching-score.test.ts``, parameterised by
      a fixture ``_FIXTURE_THRESHOLDS`` dict defined in THIS file — never Bravo's real
...
# ---------------------------------------------------------------------------
# Shared fixtures — algorithm tests use THESE thresholds, never Bravo's JSON values
...
def test_real_config_default_values_are_115_and_70():
    """Bravo's documented defaults — asserted ONLY here against the real JSON, never
