...
Split the same three ways as ``test_economy_match.py``:
  (a) ALGORITHM tests — ported from Andreas's ``tests/finance/cost-engine.test.ts``, run
      ...
      fake account numbers, never Bravo's real JSON. The accrual arithmetic must be provable
...
# change in Bravo's real chart, and the config-swap test needs a chart it can vary freely.
# ---------------------------------------------------------------------------
...
class TestConfigIsAccountsOnly:
    ...
    def test_real_bravo_config_produces_the_identical_split(self) -> None:
        # The strongest form of the claim: Bravo's REAL chart/mapping and the fixture's fake
...
class TestBravoConfigFiles:
    ...
    def test_real_config_reproduces_the_reference_case(self) -> None:
        # End-to-end with production config: Andreas's negative-WIP case, landing on Bravo's
