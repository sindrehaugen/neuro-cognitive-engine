...
- **Host Portal page served:** the service/tickets + customer-health surfaces (today the 34-line stub) — consume the no-model REST surface.
...
- **Edges (the §4 contract, our slice):**
  ...
  - `TICKET -[failure_pattern]-> PRODUCT` — **the "silence" the reference implementation flags closed**: repeated failures on a SKU flow *back* to Product(2) for better BOMs and to Sales(5) as upsell signal.
...
- **Feeds Morning-brief (#19 aggregate):** exposes the "drift gråter" signal — at-breach-risk SLAs + churn-risk customers + open proactive tickets — as the operations slice of the cross-engine "1 risk + 1 opportunity" query (the cross-module collision the reference implementation's module 19 wants to surface).
