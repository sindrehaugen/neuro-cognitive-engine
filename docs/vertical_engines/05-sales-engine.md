...
## Inspiration & triage
- **Andreas sources (lift / wire):**
  - `lib/quote/service.ts:133` `convertSignedQuoteToProject` — the **signing→project bridge**; freezes the immutable baseline (`:232-237`). 🟢 **fresh + correct but orphan (0 callers)** — Andreas built it, never wired it. We wire it (manifest `signed-quote-to-project`, angerkost 3).
  - `lib/finance/projections/signed-baseline.ts:33` `applySignedBaseline` — idempotent signed-baseline application; `marginSignedPct` is **never** overwritten by the cascade (the margin-trinity discipline). Lift the invariant.
  - `lib/pricing/index.ts` — DG-pricing (`salgspris = kostpris / (1 − DG%)`); the quote-builder bug is the inline `*0.7` that bypasses it — close that gap when we build quote pricing.
  - Spor A (quote-driven) is the canonical path; Spor B (Dynamics-Won) is the legacy we are retiring (handoff 02 §2). DealRoom / BankID-signing / AI-lead-scoring are listed *not built* in Andreas — greenfield for us.
...
- **Crown-jewel doc:** Andreas module **01 — Sales** (`handoff/04-virksomhets-modulkart.md`): Lead→quote→DealRoom→BankID→auto-project; "commission must be tied to DB/contribution-margin — it must pay to sell drift (service)".
...
- `do_convert_signed_quote_to_project(engine, params)` — wires Andreas's orphan `convertSignedQuoteToProject`: hands the frozen BOM to the Project engine (Actor; emits `SIGNED_BASELINE -[becomes]-> PROJECT`).
...
- **Serves win/loss feedback to Product:** repeated loss reasons tied to a SKU emit a `failure_pattern` signal toward Product (the "silence" Andreas flags — upsell/BOM feedback).
...
- **B3 — Quote→signature→project:** DealRoom (`do_open_dealroom`, toggle options, **shared DG-pricing service** — no inline `*0.7`, no re-impl); Scrive/Criipto signing + webhook → triggers the (already-built) freeze; **wire Andreas's orphan** `convert_signed_quote_to_project`. A2A Sales→Project handoff.
