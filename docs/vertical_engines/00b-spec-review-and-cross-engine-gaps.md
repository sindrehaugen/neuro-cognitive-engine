...
- **The Portal product sidecar is a grab-bag** (offer_*/oneflow/mailer/standards = Sales/Agreements). Make the *leave-behind* list as explicit as the lift list. The 9 "manufacturer-API adapters" are mostly **document-ingestion pipelines** (AV ships PDFs, not feeds) — reframe adapter model as *feed* vs *document* adapters.
...
- **Defers the GL reader to Economy via A2A** (never re-implements) — same ownership discipline Vendors showed. Lifts a **real Portal sidecar** (the agreement sidecar), so lower-risk than greenfield.
