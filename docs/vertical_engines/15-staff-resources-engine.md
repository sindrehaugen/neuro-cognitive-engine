...
- **No single Portal sidecar** — greenfield like Field Tech (Bravo has no resource-planning system today; allocation lives in PLs' heads). This is a build, not a lift.
...
- **Calendar sync** (Outlook/M365 — Bravo already integrates M365) for two-way push of allocations to staff calendars.
