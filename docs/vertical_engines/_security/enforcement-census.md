...
|---|---|---|---|---|---|
| **1** | **Tenant Database Isolation** | `docs/enterprise_security.md:319`<br>`docs/database_architecture.md:409`<br>`docs/multi_tenancy.md:54` | PostgreSQL RLS policies (`tenant_isolation_policy` across 92 tables in `EXPECTED_TENANT_RLS_TABLES`) | 🔴 **INERT as deployed.** Runtime connects as `mcp_user` (`rolsuper=true`, `rolbypassrls=true`). Zero policies target `mcp_user`. Isolation rests solely on application `WHERE namespace_id = $1` predicates. | 🟡 `tests/test_rls_isolation_integration.py` runs in CI, but tests against mock/isolated roles; runtime superuser bypass is not evaluated in CI. |
