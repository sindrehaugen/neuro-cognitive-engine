...
> [!NOTE]
> **CI Execution Reality (ML-CI1):** While `tests/test_c3_adversarial.py` contains 9 live integration tests proving these mitigations, the entire file is parked in `KNOWN_UNWIRED` (`tests/test_ci_integration_coverage.py:78`) and **runs in zero CI workflows**. Continuous gating in CI is queued under estate decision **Q-12** in `C:\Claude\QUESTIONS_CHARTER.md`.

| # | STRIDE | Vector | Concrete mitigation (W22/W23) | Proving test (`tests/test_c3_adversarial.py`, ML-CI1) |
|---|--------|--------|-------------------------------|-------------------------------------------------------|
