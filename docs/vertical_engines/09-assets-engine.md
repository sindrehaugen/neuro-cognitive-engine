...
- **the planning sources** (handoff `02-kode-virkelighet` §9, the most facade-heavy stage — *build the engines under the rich models*):
  ...
  - the reference implementation — **idempotent lifecycle enrichment** RECEIVED→INSTALLED→VERIFIED, sets warranty. 🟢 → lift near-1:1 into `lifecycle.py` as a pure transform; extend to the full 14-state machine.
  ...
  - the reference implementation — **driftsavtale auto-create from BOM** (matches `/driftsavtale/i`). 🟢 → seed `SLA` coverage from BOM at handover.
...
- **Host Portal page served:** the room-centric customer asset-register view (consumes the no-model REST surface).
...
## Core functions
Pure-ish `do_<action>(engine, params) -> dict`; the lifecycle transition core lifts the reference implementation's idempotent `lifecycle.ts` near-1:1 and stays pure (0 DB).
...
- `do_compute_health(engine, params) -> dict` — `{asset_id}` → **the healthScore writer the reference implementation never built**: fuses latest telemetry + NetBox MTBF (`netbox/mtbf.py`) + open `TICKET`s + age-vs-lifespan into a 0–100 score; persists on the `ASSET` node; flags DEGRADED transition past threshold.
...
- **Initiates failure-pattern feedback to Product(2):** repeated failure / short-life emits `ASSET -[failure_pattern]-> PRODUCT` — the structural close of the reference implementation's service→product silence.
