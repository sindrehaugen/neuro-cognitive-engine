...
- **Host Portal pages served:** the customer-facing shares already noted in Sales (`Motebrief.jsx`, `TilbudKunde.jsx`) are *pre-sale*; this engine owns the *post-sale* customer surface (room tracker, asset register, service-request, document, SLA self-service pages) — a **separate app** from the internal BFF.
