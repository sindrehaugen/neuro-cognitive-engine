...
## Inspiration & triage
- **Andreas source:** `04-virksomhets-modulkart.md` module **16 — HR & Performance (Bravo Academy)** — Platform axis; core objects: profilkort, skills-matrise, sertifiserings-lifecycle, kapasitet, Bravo Academy, onboarding-quest, leave. Module map §note: "Platform-modulene tjener alle andre … HR/Academy mater assignment"; "Delt ressurs-pool — 07 og 12 deler teknikere; 11 (eksterne) utvider med restricted access; 16 driver hvem som dispatches/assignes."
- **Portal sidecar to lift (`backend/bravo_hr/`):** triage each into the clean vertical —
...
- **Lysning pages served:** `BravoHr.jsx` (HR dashboard), `Brukere.jsx` (employee list), `BrukerDetalj.jsx` (profile card / skills / certs / absence), `Onboarding.jsx` (the quest) — all consume the no-model REST surface.
...
## REST routes
No-model path for the BFF (`BravoHr.jsx`, `Brukere.jsx`, `BrukerDetalj.jsx`, `Onboarding.jsx`), cron, scripts. Mounted via `build_app(extra_routes=...)`; HMAC/mTLS-authed in `nce/admin_handlers/hr.py`. **Every route enforces the access-scope check before returning PII.**
