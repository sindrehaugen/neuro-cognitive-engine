...
## Build phases
- **P1 — Lift the pure cores.** Port `phase_gates.py` (G0–G6, `VALID_PHASE_TRANSITIONS` + `GATE_CRITERIA` → config) + `baseline.py` (freeze + margin-trinity). `mcp_handlers.py` for `can_enter_phase` only. Tests for the pure functions (the spec the reference implementation never wrote).
