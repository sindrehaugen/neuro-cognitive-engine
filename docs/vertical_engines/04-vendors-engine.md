...
- **Portal sidecars to lift:** supplier master-data **rows already arriving via the Product/Procurement feeds** (the host's Nettailer client's supplier/supplierprices rows, the Portal product sidecar's bidprices module `leverandor` column). Vendors does **not** re-pull these — it **subscribes** to the canonical VENDOR upserts those feeds already produce and enriches them with the reliability layer. Contractor data is entered via the admin surface (no external feed).
- **Host Portal page(s) served:** indirectly — supplier scorecards surface on `Bestillinger.jsx` (procurement) sourcing rationale; contractor matching + restricted partner views feed the Field Tech dispatch surface.
