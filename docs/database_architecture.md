...
* **RLS Surface & Catalog Verification**: The authoritative tenant isolation surface encompasses 118 tables defined in `EXPECTED_TENANT_RLS_TABLES` (`nce/event_log.py`). At server startup, `verify_rls_catalog_consistency()` asserts that every tenant table has active RLS (`relrowsecurity = true`) and carries the required `tenant_isolation_policy` policy. Shared reference tables without RLS (`product_catalog`, plus 5 platform tables) are defined in `EXPECTED_GLOBAL_TABLES`. Confidential commercial pricing (`product_prices`) remains tenant-isolated.
* **Admin Bypass (`unmanaged_pg_connection`)**: A narrow set of global background maintenance operations check out connections via `unmanaged_pg_connection(pool, *, site=...)` (`nce/db_utils.py:145-160`), which skips `SET LOCAL nce.namespace_id`. Every call site must provide a registered string validated against the 22 audited sites in `UNMANAGED_PG_AUDITED_SITES` (`nce/db_utils.py:24-60`). Unregistered sites raise `ValueError` at runtime.
...
> **Authoritative DDL Source:** The canonical DDL is maintained in [`nce/schema.sql`](https://github.com/sindrehaugen/NCE/blob/main/nce/schema.sql) and the sequential migration chain in [`nce/migrations/`](https://github.com/sindrehaugen/NCE/tree/main/nce/migrations/) (`001_enable_rls.sql` through `095_site_master_data.sql`).
>
...
### 9b. Complete Versioned Migrations Catalog (001 through 083)
...
| [`065_support_service_tickets.sql`](https://github.com/sindrehaugen/NCE/blob/main/nce/migrations/065_support_service_tickets.sql) | Support Engine (Module 10, Wave 1): native ServiceTicket store (`service_tickets`), fast SLA countdown clocks (`sla_clocks`), and rolling customer health scoring (`customer_health`). | `service_tickets`, `sla_clocks`, `customer_health` | None | `tenant_isolation_policy` on all three tables |
...
| [`067_field_tech_work_orders.sql`](https://github.com/sindrehaugen/NCE/blob/main/nce/migrations/067_field_tech_work_orders.sql) | Field Tech Engine (Module 12, Wave 1): native work orders (`work_orders`), ISO9001 checklists (`checklists`), and time entries (`time_entries`). | `work_orders`, `checklists`, `time_entries` | None | `tenant_isolation_policy` on all three tables |
| [`068_hr_engine.sql`](https://github.com/sindrehaugen/NCE/blob/main/nce/migrations/068_hr_engine.sql) | HR Engine (Module 13, Wave 1): native employees (`employees`), skills matrix (`skills`), certifications lifecycle (`certifications`), and absence tracking (`absences`). | `employees`, `skills`, `certifications`, `absences` | None | `tenant_isolation_policy` on all four tables |
| [`069_marketing_engine.sql`](https://github.com/sindrehaugen/NCE/blob/main/nce/migrations/069_marketing_engine.sql) | Marketing Engine (Module 14, Wave 1): case studies (`case_studies`), customer testimonials with dual-tier consent (`testimonials`), and content assets with AEO/GEO metadata (`content_assets`). | `case_studies`, `testimonials`, `content_assets` | None | `tenant_isolation_policy` on all three tables |
| [`070_resources_registry.sql`](https://github.com/sindrehaugen/NCE/blob/main/nce/migrations/070_resources_registry.sql) | Staff & Resources Engine (Module 15, Phase 1): schedulable resources registry (`resources`) over people, contractors, vehicles, and tools. | `resources` | None | `tenant_isolation_policy` |
| [`071_allocations_btree_gist.sql`](https://github.com/sindrehaugen/NCE/blob/main/nce/migrations/071_allocations_btree_gist.sql) | Staff & Resources Engine (Module 15, Phase 2): allocations with btree_gist conflict exclusion, travel_legs, stays, and statutory per_diems. | `allocations`, `travel_legs`, `stays`, `per_diems` | None | `tenant_isolation_policy` |
| [`072_business_insights_engine.sql`](https://github.com/sindrehaugen/NCE/blob/main/nce/migrations/072_business_insights_engine.sql) | Business Insights Engine (Module 16, Wave 1): cached KPI snapshots (`business_insights_kpi_snapshots`) for executive cockpit and performance trends. | `business_insights_kpi_snapshots` | None | `tenant_isolation_policy` on `business_insights_kpi_snapshots` |
...
| [`074_procurement_po_lines.sql`](https://github.com/sindrehaugen/NCE/blob/main/nce/migrations/074_procurement_po_lines.sql) | Procurement Engine (Module 1, Wave PR-2): purchase order lines and status state machine (`procurement_po_lines`). | `procurement_po_lines` | None | `tenant_isolation_policy` |
| [`075_decision_feedback.sql`](https://github.com/sindrehaugen/NCE/blob/main/nce/migrations/075_decision_feedback.sql) | Decision Feedback Service (Phase 2, Wave C10): cross-engine human decision and outcome telemetry (`decision_feedback`). | `decision_feedback` | None | `tenant_isolation_policy` on `decision_feedback` |
| [`076_signing_credentials.sql`](https://github.com/sindrehaugen/NCE/blob/main/nce/migrations/076_signing_credentials.sql) | Signing Service (Sales / Cross-Engine Wave Q-4): encrypted broker signing credentials per tenant namespace (`signing_credentials`). | `signing_credentials` | None | `tenant_isolation_policy` on `signing_credentials` |
| [`077_event_log_chain_hash_present.sql`](https://github.com/sindrehaugen/NCE/blob/main/nce/migrations/077_event_log_chain_hash_present.sql) | Tamper-evidence hardening: requires `chain_hash` on every NEW `event_log` row. `signature` and `signature_key_id` were already `NOT NULL` but `chain_hash` was not, so a direct INSERT could write a signed event with no chain hash — which is what left one namespace unverifiable. Added `NOT VALID` because exactly one historical row violated it when this migration ran, and clearing that row was in no sense routine: `trg_event_log_worm` forbids DELETE even for a superuser, and `event_log_namespace_id_fkey` blocks removing the namespace. Both of those are still true. That row was removed once, on 2026-09-10, under Sindre's explicit authorisation, by suspending the WORM trigger inside the same transaction that re-armed it — a one-off, not an available maintenance operation. 078 then promoted this constraint to validated; its row below has the detail. | None | `event_log` (CHECK constraint only) | `event_log_chain_hash_present` (CHECK, added `NOT VALID`; validated by 078) |
| [`078_event_log_chain_hash_validate.sql`](https://github.com/sindrehaugen/NCE/blob/main/nce/migrations/078_event_log_chain_hash_validate.sql) | Promotes `event_log_chain_hash_present` from `NOT VALID` to **validated**, so the chain-hash rule is an invariant PostgreSQL has actually verified rather than an intent enforced only for new rows. 077 could not do this: exactly one historical row violated it and could not be removed under WORM. That row -- test litter written directly into `event_log` by a fixture bypassing `append_event`, which is what left one namespace unverifiable -- was removed 2026-09-10 with explicit authorisation, so the estate now has **zero** NULL `chain_hash` rows. `VALIDATE` on the partitioned parent recurses to all five partitions (verified). Does **not** fix `(namespace_id, event_seq)` uniqueness -- the partition-key requirement still prevents it, and that remains a code guard (#128's chokepoint ratchet), not a database guarantee. | None | `event_log` (constraint validation only) | `event_log_chain_hash_present` (CHECK, now **VALID**) |
| [`079_system_design_sidetable_fk_cascade.sql`](https://github.com/sindrehaugen/NCE/blob/main/nce/migrations/079_system_design_sidetable_fk_cascade.sql) | Closes D12: ties System Design side-tables (`system_design_device_capabilities`, `system_design_geometry`, `system_design_node_state`) to `kg_nodes` with composite FK `(node_label, namespace_id) REFERENCES kg_nodes(label, namespace_id) ON DELETE CASCADE`. | None | `system_design_device_capabilities`, `system_design_geometry`, `system_design_node_state` | `fk_sddc_kg_nodes`, `fk_sdg_kg_nodes`, `fk_sdns_kg_nodes` (all `ON DELETE CASCADE`) |
| [`080_worm_truncate_guard.sql`](https://github.com/sindrehaugen/NCE/blob/main/nce/migrations/080_worm_truncate_guard.sql) | RL-H18: the WORM guard could not stop a `TRUNCATE`. `trg_event_log_worm` and `trg_event_parents_worm` are `BEFORE UPDATE OR DELETE ... FOR EACH ROW`, and a row-level trigger is invoked once per affected row -- `TRUNCATE` never visits rows, it deallocates the table's storage, so it fired no trigger at all and either append-only hash-chained table could be emptied in a single statement with the guard silent. Not theoretical on this deployment: the connecting role (`mcp_user`) is `rolsuper`/`rolbypassrls`, so it holds `TRUNCATE` on both tables and RLS does not constrain it. Adds a statement-level `BEFORE TRUNCATE` trigger to each -- the only shape PostgreSQL fires for that operation. `prevent_mutation()` already reports `TG_OP`, so it raises "TRUNCATE operation is forbidden" with no function change; the existing row triggers are untouched. Verified against a real Postgres and mutation-proven: dropping the new trigger makes the guard test fail by the `TRUNCATE` succeeding. | None | `event_log`, `event_parents` (triggers only) | `trg_event_log_worm_truncate`, `trg_event_parents_worm_truncate` (both `BEFORE TRUNCATE ... FOR EACH STATEMENT`) |
| [`081_schema_sql_only_ddl_baseline.sql`](https://github.com/sindrehaugen/NCE/blob/main/nce/migrations/081_schema_sql_only_ddl_baseline.sql) | Q-4 step 1: gives the migration ledger a path to the DDL that only `schema.sql` provides. `NCEEngine.connect()` runs two schema writers (`orchestrator.py:205-206`) -- the whole of `schema.sql`, then the numbered chain. `schema.sql` is **not** a dead second writer: its unguarded `ADD COLUMN` statements sit inside `DO $$` blocks that probe `information_schema` first, so it detects missing columns and adds them on every connect. It is a migration runner that keeps no ledger, and that is the real defect -- nothing can state what schema version a live database is at. Measured on the live database 2026-09-17: `applied_migrations` held 74 rows (max `078`) while all ten columns below were present and unrecorded. This migration is a **no-op on any recently-connected database by design**; it is a baseline, not a change. It matters for Q-4 step 2: once `_init_pg_schema` becomes bootstrap-only, an existing database stops receiving anything that lives solely in `schema.sql`, and without this there would be no ledgered path to these columns. **Step 2 must not land before this has.** Scope is columns only -- the core tables (`memories`, `kg_nodes`, `kg_edges`, `event_log`, `namespaces`) are created in `schema.sql` and in zero migrations, and `schema.sql` remains the bootstrap for an empty database. Verified on a throwaway database: clean no-op on a current schema, idempotent across two runs, and it repairs a database whose columns were dropped (both restored, zero NULL `namespace_id` left). | None | `kg_nodes`, `kg_edges`, `memories`, `consolidation_runs`, `bridge_subscriptions` | None -- `ADD COLUMN IF NOT EXISTS` only, plus the `_global_legacy` namespace backfill that `schema.sql` performs, reproduced so a database reaching this migration ends in the same state rather than a NULL-bearing one |
| [`082_schema_sql_only_rls_baseline.sql`](https://github.com/sindrehaugen/NCE/blob/main/nce/migrations/082_schema_sql_only_rls_baseline.sql) | Q-4 step 1b: gives the ledger a path to the row-level security that only `schema.sql` provides. 081 covered the ten schema-only columns; that was not enough, and the gap was in the checker's scope rather than in the data. Measured on `f2dba69`: `CREATE POLICY` — schema.sql 50, migrations 65, **4 only in schema.sql**; `ENABLE ROW LEVEL SECURITY` — schema.sql 49, migrations 64, **4 only in schema.sql**. The four are `outbox_events`, `replay_runs`, `saga_execution_log` and `topology_graph`. This matters more than the columns did: after Q-4 step 2 a missing column eventually raises an error at query time, whereas a missing policy raises nothing — it is one tenant reading another tenant's rows with every test still green. **Step 2 must not land before this has.** No-op on the live database (verified 2026-09-17: all four `relrowsecurity = true`, one policy each). `topology_graph`'s policy is reproduced **verbatim including its weakness** — it has no `WITH CHECK`, so it constrains reads but not writes; tightening it inside a baseline would be a behaviour change wearing a no-op's clothing, so it is filed as its own row and pinned by `test_migration_082_reproduces_topology_graph_weakly_on_purpose`. Note three of the four grant `TO nce_app`, a role nothing connects as — the deployed role is `mcp_user` (`rolsuper`/`rolbypassrls`), so these policies do not constrain the running system either way; that is the standing RLS-posture finding, unchanged here. | None | `outbox_events`, `replay_runs`, `saga_execution_log`, `topology_graph` (RLS + policy only) | `tenant_isolation_policy` on the first three, `topology_graph_tenant_isolation` on the fourth; all created inside `IF NOT EXISTS` guards against `pg_policy` |
| [`083_notifications_reminders.sql`](https://github.com/sindrehaugen/NCE/blob/main/nce/migrations/083_notifications_reminders.sql) | C13 Notifications and Reminders (Wave A-3): persistent multi-tenant user notification inbox (`notifications`), user-set reminder engine (`reminders`), and subscription registry (`notification_subscriptions`). | `notifications`, `reminders`, `notification_subscriptions` | None | `tenant_isolation_policy` on all three tables |
| [`084_document_register.sql`](https://github.com/sindrehaugen/NCE/blob/main/nce/migrations/084_document_register.sql) | C14 Document Register (Wave A-4): master document metadata and storage references (`documents`), polymorphic entity graph edges (`document_links`), and expiring share tokens (`document_shares`). | `documents`, `document_links`, `document_shares` | None | `tenant_isolation_policy` on all three tables |
| [`085_legal_entity_register.sql`](https://github.com/sindrehaugen/NCE/blob/main/nce/migrations/085_legal_entity_register.sql) | C15 Legal-Entity Register (Wave A-5): multi-tenant legal entity register (`legal_entities`) with org.nr, parent group relations, and roles. | `legal_entities` | None | `tenant_isolation_policy` |
| [`086_geodata_osm_elements.sql`](https://github.com/sindrehaugen/NCE/blob/main/nce/migrations/086_geodata_osm_elements.sql) | Geodata FEED module (Lane F Wave F-11): local store for OpenStreetMap elements (`geodata_osm_elements`), keyed `(osm_type, osm_id)`, with a `box`-typed bounding-box column and GiST index for viewport intersection reads (no PostGIS in this image). GLOBAL reference data, same reasoning as `product_catalog` (migration 064) -- numbered 086, not the originally-claimed 085, after Lane A's #240 landed as 085 while this PR was still open. | `geodata_osm_elements` | None | None (table moves to `EXPECTED_GLOBAL_TABLES`, no RLS) |
| [`087_assets_shell_product.sql`](https://github.com/sindrehaugen/NCE/blob/main/nce/migrations/087_assets_shell_product.sql) | Assets Engine (Wave D-1): Asset coverage exclusion shell flag (`is_shell`), global product links (`product_id`), and SKU denormalisation (`product_sku`). | None | `assets` | `tenant_isolation_policy` |
| [`088_sales_resource_tables.sql`](https://github.com/sindrehaugen/NCE/blob/main/nce/migrations/088_sales_resource_tables.sql) | C12 Sales Resource Surface (Wave B-1): relational tables for customer accounts, inbound leads, deal opportunities, and quotes (`sales_customers`, `sales_leads`, `sales_deals`, `sales_quotes`). | `sales_customers`, `sales_leads`, `sales_deals`, `sales_quotes` | None | `tenant_isolation_policy` on all four tables |
| [`089_geodata_n50_land_cover.sql`](https://github.com/sindrehaugen/NCE/blob/main/nce/migrations/089_geodata_n50_land_cover.sql) | Geodata FEED module (Lane F Wave F-12): local store for Kartverket's N50 land-cover classification (`geodata_n50_land_cover`), keyed `(klasse, objid)`, with a `box`-typed bounding-box column and GiST index for viewport intersection reads (no PostGIS in this image). GLOBAL reference data, same reasoning as `product_catalog` (migration 064) -- numbered 089, not 085-088, to avoid the four-way collision those numbers hit on PRs #239/#240/#242/#245; see the migration's own header. | `geodata_n50_land_cover` | None | None (table moves to `EXPECTED_GLOBAL_TABLES`, no RLS) |
| [`090_geodata_place_names.sql`](https://github.com/sindrehaugen/NCE/blob/main/nce/migrations/090_geodata_place_names.sql) | Geodata FEED module (Lane F Wave F-13): local store for Kartverket's place-name register (`geodata_place_names`), one anchor `point` per named place, with a GiST index supporting `<->` nearest-neighbour lookup (no PostGIS in this image; a bounding-box shape does not answer "nearest place" correctly -- see the migration's own header). GLOBAL reference data, same reasoning as `product_catalog` (migration 064) -- numbered 090 (087/088 already reserved for PRs #242/#245). | `geodata_place_names` | None | None (table moves to `EXPECTED_GLOBAL_TABLES`, no RLS) |
| [`091_support_ticket_actions.sql`](https://github.com/sindrehaugen/NCE/blob/main/nce/migrations/091_support_ticket_actions.sql) | Support Engine (Module 10, Wave D-5 -- TICKET_ACTION log): append-only ticket action log tracking interventions (*tiltak*) and outcomes (*utfall*) per ticket per ADR-0008. | `support_ticket_actions` | None | `tenant_isolation_policy` on `support_ticket_actions` (strict forced RLS) |
| [`092_pricing_fx_rates.sql`](https://github.com/sindrehaugen/NCE/blob/main/nce/migrations/092_pricing_fx_rates.sql) | Shared pricing service FX feed (Lane F Wave F-15): last-known-good persistence (`pricing_fx_rates`, one row per currency, upserted) for `nce/pricing/fx.py`'s Norges Bank EXR adapter, so a process restart is not a cold start. GLOBAL reference data, same reasoning as `product_catalog` (migration 064) -- numbered 092, not 091, reserved for PR #252 (Lane D) per ML-orch ruling (090 is this same lane's own #258, merged). | `pricing_fx_rates` | None | None (table moves to `EXPECTED_GLOBAL_TABLES`, no RLS) |
| [`093_principal_bindings.sql`](https://github.com/sindrehaugen/NCE/blob/main/nce/migrations/093_principal_bindings.sql) | C16 Principal Mapping (Wave A-6): multi-tenant principal bindings (`principal_bindings`) mapping caller principal_id to employee/customer/contractor identities and roles. | `principal_bindings` | None | `tenant_isolation_policy` |
| [`094_outbound_webhooks.sql`](https://github.com/sindrehaugen/NCE/blob/main/nce/migrations/094_outbound_webhooks.sql) | C4 Outbound Webhooks (Wave A-7): outbound webhook subscriptions (`outbound_webhooks`) with event selector matching, HMAC-SHA256 request signing, and transactional outbox relay delivery. | `outbound_webhooks` | None | `tenant_isolation_policy` |
| [`095_site_master_data.sql`](https://github.com/sindrehaugen/NCE/blob/main/nce/migrations/095_site_master_data.sql) | C17 Site Master Data (Wave A-9): multi-tenant site and building master data register (`sites`) with cadastre identity, validated address, coordinates, height, and footprint geometry. | `sites` | None | `tenant_isolation_policy` |
| [`096_agreement_parties_and_resources.sql`](https://github.com/sindrehaugen/NCE/blob/main/nce/migrations/096_agreement_parties_and_resources.sql) | C12 Agreements Resource Surface (Wave B-9): relational tables for agreements, parties/signatures, and templates (`agreements`, `agreement_parties`, `agreement_templates`). | `agreements`, `agreement_parties`, `agreement_templates` | None | `tenant_isolation_policy` on all three tables |
| [`097_sales_contacts.sql`](https://github.com/sindrehaugen/NCE/blob/main/nce/migrations/097_sales_contacts.sql) | C12 Sales CONTACT Resource (Wave B-2): satellite table (`sales_contacts`, name/email/phone) for the kg_nodes-primary CONTACT node type, FK'd to `kg_nodes(label, namespace_id)` following the DEVICE/PORT/RACK/CABLE satellite pattern (migration 079). No `customer_id`/`sales_customers` link (Q-46, open). | `sales_contacts` | None | `tenant_isolation_policy` |
| [`098_source_mode_heartbeat.sql`](https://github.com/sindrehaugen/NCE/blob/main/nce/migrations/098_source_mode_heartbeat.sql) | C5 flip-gate staleness heartbeat (follow-on from Wave D-9's own stated limitation): one row per `(namespace_id, engine)`, upserted on every "both"-mode `resolve()` call, making "did a comparison actually run" observable independent of whether it found a divergence. Surfaced on `flip_status()` as `last_compared_at`/`heartbeat_stale`; deliberately not wired into `flip_function`'s authorization decision (a separate, explicit choice). | `source_mode_heartbeat` | None | `tenant_isolation_policy` |
| [`099_sales_quote_billing_method.sql`](https://github.com/sindrehaugen/NCE/blob/main/nce/migrations/099_sales_quote_billing_method.sql) | Charter Wave B-5: `billing_method` attribute on QUOTE (`sales_quotes`), nullable `TEXT` with `CHECK (billing_method IS NULL OR billing_method IN ('percent', 'hours_amount'))`. Nullable with no default -- nothing anywhere reads or computes a billing method yet (confirmed against `commission.py`), so `NULL` states "not yet classified" for every historical quote rather than asserting a fact nothing recorded. | None | `sales_quotes` | `tenant_isolation_policy` (unchanged, already present) |
| [`100_procurement_deal_registrations.sql`](https://github.com/sindrehaugen/NCE/blob/main/nce/migrations/100_procurement_deal_registrations.sql) | Procurement Engine (Wave B-15): supplier deal registrations (`procurement_deal_registrations`) protecting pricing/terms for a specific opportunity with a named supplier. `supplier` is plain TEXT, matching `procurement_bid_prices.leverandor`'s precedent -- no suppliers table exists anywhere in this schema. | `procurement_deal_registrations` | None | `tenant_isolation_policy` |
| [`101_quote_templates_and_packages.sql`](https://github.com/sindrehaugen/NCE/blob/main/nce/migrations/101_quote_templates_and_packages.sql) | Charter Wave B-7: reusable quote starting-point templates (`sales_quote_templates`, Sales-owned) and package catalog definitions (`product_packages`, Product-owned, `enabled_guard=require_product_enabled`). Both genuinely new node types -- no prior quote-template or package-catalog infrastructure existed. `template_lines`/`components` are JSONB static content, not normalised child tables. | None | `sales_quote_templates`, `product_packages` | `tenant_isolation_policy` on both tables |
| [`102_billing_runs.sql`](https://github.com/sindrehaugen/NCE/blob/main/nce/migrations/102_billing_runs.sql) | Economy Engine (Wave B-12): kg_nodes-primary satellite (`economy_billing_runs`) for the new BILLING_RUN node type, grouping the BILLING_CANDIDATEs generated together for one namespace + billing period. FK'd to `kg_nodes(label, namespace_id)` following the `sales_contacts`/`097` satellite pattern. | `economy_billing_runs` | None | `tenant_isolation_policy` |
| [`103_billing_candidates.sql`](https://github.com/sindrehaugen/NCE/blob/main/nce/migrations/103_billing_candidates.sql) | Economy Engine (Wave B-12): kg_nodes-primary satellite (`economy_billing_candidates`, one consolidated candidate invoice per agreement per period) for the new BILLING_CANDIDATE node type, plus its plain (non-kg_nodes) child table (`economy_billing_candidate_lines`, one row per priced C-2 room-category tier). | `economy_billing_candidates`, `economy_billing_candidate_lines` | None | `tenant_isolation_policy` on both tables |
| [`104_system_design_design_requests.sql`](https://github.com/sindrehaugen/NCE/blob/main/nce/migrations/104_system_design_design_requests.sql) | C12 DESIGN_REQUEST resource surface (charter Wave C-4): satellite table (`system_design_design_requests`) for the kg_nodes-primary DESIGN_REQUEST node type, FK'd to `kg_nodes(label, namespace_id)` following the CONTACT/097 pattern. Replaces the prior `system_design_geometry.meta` JSONB scheme (which had no kg_nodes row at all); backfills any pre-existing rows under the old scheme into both the new table and a kg_nodes identity row, idempotently. | `system_design_design_requests` | None | `tenant_isolation_policy` |
| [`105_customer_invoices.sql`](https://github.com/sindrehaugen/NCE/blob/main/nce/migrations/105_customer_invoices.sql) | Economy Engine (Wave B-13): kg_nodes-primary satellite (`economy_customer_invoices`) for the new CUSTOMER_INVOICE node type -- one invoice per BILLING_CANDIDATE, F13 lifecycle (proposal -> approved -> exported -> paid), this wave ships the proposal transition only. Deliberately distinct from the existing bare INVOICE node type (the supplier-side PO -[posted_to]-> INVOICE boundary edge). | `economy_customer_invoices` | None | `tenant_isolation_policy` |
| [`106_product_lifecycle_status_check.sql`](https://github.com/sindrehaugen/NCE/blob/main/nce/migrations/106_product_lifecycle_status_check.sql) | Product Engine (Q-49): closes `product_catalog.lifecycle_status` (free text since `031`) to a real vocabulary -- `'coming'`/`'active'`/`'EOL'`/`'EOS'`. Backfills any non-conforming existing value by mapping the estate's own already-stated synonyms (`watchers.py`'s dead 5-synonym EOL scan, `related.py`'s 3-value set) onto the closed set, defaulting anything unrecognised to `'active'`, then adds the CHECK constraint. No new column and no new table -- the column already existed. | None | `product_catalog` (CHECK constraint only) | `product_catalog_lifecycle_status_check` (CHECK) |
| [`107_sales_deal_participants.sql`](https://github.com/sindrehaugen/NCE/blob/main/nce/migrations/107_sales_deal_participants.sql) | C12 DEAL_PARTICIPANT resource (charter Wave B-3, sub-resource half): plain relational child table (`sales_deal_participants`) FK'd to `sales_deals(id)`, following the `agreement_parties`/096 pattern rather than PO_LINE (sales_deals has a real UUID `id`, so a direct FK is the correct shape). No kg_nodes involvement -- table-backed like DEAL/AGREEMENT_PARTY/PO_LINE. | `sales_deal_participants` | None | `tenant_isolation_policy` |
| [`108_support_ticket_actions_append_only.sql`](https://github.com/sindrehaugen/NCE/blob/main/nce/migrations/108_support_ticket_actions_append_only.sql) | ADR-0008: enforces the append-only invariant `support_ticket_actions` was documented to have since migration 091 but never had -- revokes `UPDATE`/`DELETE` on `support_ticket_actions`, narrowing its grant to `SELECT, INSERT` (matching `event_log`'s own shape, not its trigger; see the ADR for why grant-level is proportionate here). No new column and no new table -- grant change only. | None | `support_ticket_actions` (grant only) | `tenant_isolation_policy` (unchanged, already present) |

### 9c. Automated RLS Catalog Consistency Gate
To guarantee that newly added tables cannot be deployed without tenant isolation, NCE executes `verify_rls_catalog_consistency()` ([`nce/event_log.py`](https://github.com/sindrehaugen/NCE/blob/main/nce/event_log.py)) during startup. The validator compares live PostgreSQL catalog metadata (`pg_tables`, `pg_class`, `pg_policies`) against the 118 tables in `EXPECTED_TENANT_RLS_TABLES`:
* Asserts that `relrowsecurity` is enabled on all 118 tenant tables.
