...
- **The signed-baseline freeze is append-only by database natural key and DDL grant** (§7.1) — DDL revokes `UPDATE`/`DELETE` from `nce_app`, backed by a `UNIQUE (namespace_id, quote_id)` constraint. Note deployment reality: runtime connects as superuser `mcp_user`, which bypasses PostgreSQL role-level table grants; the active barriers are the database `UNIQUE` constraint and application idempotency checks.
...
- **Idempotency on the signing/freeze path is hand-rolled, not governor-provided.** `do_freeze_baseline` checks for an existing row before inserting (`baseline.py:129-150`); `do_on_signed_callback` checks `manual.signing_status` before re-running (`signing.py:157-168`). These are correct and tested in code, but note that their integration test suites (`tests/test_sales_signed_baseline.py`, `tests/test_sales_sign_to_project.py`) are parked in `KNOWN_UNWIRED` (ML-CI1) and do not currently run in CI. They are bespoke idempotency checks per function, not the shared `idempotency_key` + governor-cache pattern Product uses.
