...
The **Customer Portal Engine** (`nce/vertical_modules/customer_portal/`) is NCE's **external, customer-facing surface** — planning module 13, "Customer Portal," in the source module map. Its organising principle is **room-centric, not project-centric**: a customer sees *"your boardroom is 80% ready,"* never an internal project number, via a Domino's-style tracker (`Planned → Ordered → Delivered → Installed → Tested → Ready`, plus an explicit unweighted `Not Tracked` stage). Per the design intent recorded in `docs/vertical_engines/17-customer-portal-engine.md`, the engine is meant to become **more valuable after handover** — a room-centric asset register, service-request intake, document access (FDV/as-built), SLA self-service, and expansion/re-buy capture all keep the customer returning long after delivery. This guide covers the **9 registered MCP tools**; the companion admin guide covers enablement, the dedicated container deployment, RLS, and a live drift finding.

> [!IMPORTANT]
> **Surface summary (`main` @ `e00118e`):**
> - **9 exposed MCP tools**, all registered `nce/tool_registry.py`. **None** are `admin_only`; 6 are `cacheable`; 3 are `mutation`.
> - **12 REST routes** mounted on the engine's dedicated container deployment (`portal_server.py`, port 8005 behind Caddy `/api/portal/*` with 10MB payload limit, Wave CP-5) — separate from the internal admin app.
...
Projects the Domino's tracker state for a single room: `stage`, `percent_ready`, a customer-safe `status` label, and a plain-language `summary`. The caller supplies `room_id`, `room_name`, `site_id`/`site_name`, and the raw `bom_lines`/`assets` lists (each carrying a `status`/`lifecycle` field); `compute_room_stage_and_progress` (`rooms.py:30-86`) maps each line through `room-tracker-stages.json`'s `bom_status_to_stage`/`asset_lifecycle_to_stage` tables, averages the resulting stage weights, and reports the *lowest* matched stage the average percentage clears.

`room-tracker-stages.json` defines 6 weighted stages (`planned`: 10%, `ordered`: 25%, `delivered`: 50%, `installed`: 75%, `tested`: 90%, `ready`: 100%) plus a 7th unweighted stage **`not_tracked`** (display name *"Not tracked"*, 0% progress) added in Wave CP-3 (PR #205). If BOM lines or assets are unmapped, empty, or marked `not_tracked`, the tracker honestly renders **0% / "Not tracked"** instead of falling back to the silent 10% / "Planned" assumption. **When to call it:** a customer or a portal front-end asking "how is my boardroom coming along?"
