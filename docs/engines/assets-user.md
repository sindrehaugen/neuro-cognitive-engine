...
All 7 real adapters declare explicit `_ALLOWED_READS` (method, path) allowlist literals and refuse unauthorized requests before any transport call. Implementation status is machine-gated by `tests/test_advertised_capability_ratchet.py` (Wave H-11, PR #219; revised Q-38 part 1, 2026-09-19). Three platforms are absent (`crestron`, `sennheiser`, `shure`, raising `NotImplementedError` via `UnimplementedVendorAdapter`) — each previously resolved to a dedicated class that called a hardcoded, never-verified endpoint with no vendor documentation behind it, and Sindre's Q-38 part 1 ruling authorized deleting those three files rather than leaving code that looked implemented but wasn't. `huddly`/`poly` are no longer platform keys at all (dropped in the same ruling; neither ever had a dispatch branch or an NCE consumer).
