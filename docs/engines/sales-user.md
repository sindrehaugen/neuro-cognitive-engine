...
2. **Database-level natural key & WORM grant.** A `UNIQUE (namespace_id, quote_id)` constraint backs the natural key (`nce/schema.sql:1642`). The migration that creates the table also revokes all privileges from role `nce_app` and grants back only `SELECT, INSERT` — no `UPDATE`, no `DELETE` (`nce/migrations/041_sales_signed_baselines.sql:29-33`; mirrored in `nce/schema.sql:2100-2114`). Note deployment and CI reality: (a) runtime connections execute as `mcp_user` (`rolsuper = true`), which bypasses PostgreSQL role-level table grants, making the application-level idempotency check and the database `UNIQUE` constraint the active immutability barriers; (b) the test suite asserting `nce_app` privilege rejection (`tests/test_sales_signed_baseline.py:135-182`) is parked in `KNOWN_UNWIRED` (ML-CI1) and does not currently run in CI.
...
1. **`do_request_signature(engine, params)`** (`signing.py`) — params: `namespace_id`, `quote_id` (required), `signer` (required dict with `name` and valid `email`), `method` (one of `"manual" | "email_code" | "oneflow" | "criipto" | "signicat"`, defaults to `"manual"`). Deterministically renders quote document from the frozen baseline (ruling b). Resolves the transport via `get_signing_transport(method)`. Stamps the quote's `manual` JSON in `sales_read_model` with `signing_session_id`, `signing_status: "pending"`, `signing_method`, `signing_security_tier`, `signing_fingerprint`, `document_hash`, `signer_name`, `signer_email`.
